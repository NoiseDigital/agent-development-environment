"""Service-to-service auth for internal Cloud Run hops.

mcp-stats calls the agent service (resolve.py looks up an upload's storage key)
which runs with INTERNAL-only ingress. Cloud Run requires the caller to present
a Google-signed OIDC ID token whose audience is the target service URL; routing
through the VPC alone is rejected (403). This mints that token.

Off-GCP (local dev, tests) the agent URL is plain `http://` with no metadata
server, so `id_token_for` returns None and the caller falls back to an
...
_TTL_SECONDS = 50 * 60  # tokens live ~60 min; refresh early
