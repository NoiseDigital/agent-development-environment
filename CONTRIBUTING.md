...
| `mcp` | MCP services/configs (`_images/*`, `math`, and toolbox tool definitions) |
...
1. Add `source`, `tool`, and `toolset` documents to `services/backend/mcp/_images/toolbox/tools.yaml`
...
- Image-based MCPs live under `services/backend/mcp/_images/<name>/` (configs, `.env.example`, optional thin-wrapper Dockerfile)
